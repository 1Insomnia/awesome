...
local time = {
    ...
    },
    {
        nil,
        {
            make_little_dot(beautiful.red),
            make_little_dot(beautiful.cyan),
            make_little_dot(beautiful.green),
            spacing = dpi(10),
...
local reset_search_icon = function ()
    search_icon.markup = helpers.colorize_text("", beautiful.yellow)
end
reset_search_icon()
...
function sidebar_activate_prompt(action)
    ...
    elseif action == "web_search" then
        prompt = generate_prompt_icon("", beautiful.cyan)
    end
